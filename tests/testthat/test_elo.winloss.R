...
test_that("elo.winloss is working correctly", {
  ...
  tmp.wl.wt <- elo.winloss(score(points.Home, points.Visitor) ~ team.Home + team.Visitor, data = trn, weights = 1:51)
  ...
  tmp.wl.players <- elo.winloss(score(points.Home, points.Visitor) ~ players(team.Home, "Zealous Zebras") + players(team.Visitor, "Yelling Yaks"), data = trn)
